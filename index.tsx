  ...
  const { sortedTransactions, balanceData, periodicalSummary, weeklyCategoryTotals, transactionsWithBalance } = useMemo(() => {
    ...
    const withBalance = [...transactions]
        .sort((a, b) => {
            ...
            // NEW: Expense Sorting Logic
            // If both are expense transactions
            if (a.type === 'expense') {
                // Sort by Category Z-A (so A-Z after reverse in the final list)
                // b vs a comparison for 'ko' means Descending order in this step.
                // When reversed at the end, it becomes Ascending (Ga-Na-Da).
                const catCompare = b.category.localeCompare(a.category, 'ko');
                if (catCompare !== 0) return catCompare;
                
                // Sort by Memo Z-A (so A-Z after reverse in the final list)
                const memoA = a.memo || '';
                const memoB = b.memo || '';
            ...
            // For expenses fallback, or as a final fallback for incomes, sort by creation order (ID).
            return a.id - b.id;
        })
        .map(tx => {
    ...
    // 요청에 따른 경상비 순서: 십일조, 주일헌금(주정헌금), 감사헌금, 절기헌금
    // 생일감사, 심방감사, 일천번제 제외
...
}> = ({ weeklyGyeongsangbiBreakdown, yearlyGyeongsangbiBreakdown, weeklySeongyo, weeklyGeonchuk, yearlySeongyo, yearlyGeonchuk }) => {
    // 순서 지정: 십일조, 주정헌금, 감사헌금, 절기헌금
    ...
    return (
        <section className="card periodical-summary category-breakdown-summary">
            ...
            <div className="row-values gyeongsangbi">
                {displayOrder.map(category => (
                    ...
                    <span style={{ fontSize: '0.8rem', color: '#888', marginTop: '2px' }}>
                        (년: {yearlyGyeongsangbiBreakdown[category].toLocaleString()})
                    </span>
                    </div>
                ))}
            </div>
            ...
            <div className="row-values">
                ...
                <span style={{ fontSize: '0.8rem', color: '#888', marginTop: '2px' }}>
                    (년: {yearlySeongyo.toLocaleString()})
                </span>
                </div>
                <div className="value-item">
                ...
                <span style={{ fontSize: '0.8rem', color: '#888', marginTop: '2px' }}>
                    (년: {yearlyGeonchuk.toLocaleString()})
                </span>
                </div>
            </div>
            </div>
                    ...
                    {searchType === 'amount' && (
                        <div className="form-group">
                            ...
                            <input 
                                ...
                                value={amountQuery} 
                                onChange={e => setAmountQuery(e.target.value === '' ? '' : Number(e.target.value))} 
                            />
                        </div>
                    )}
                </div>
                <div className="search-results">
                    ...
                    {amountSearchResult && (
                        <>
                           ...
                           <ul>{amountSearchResult.transactions.map(tx => (
                               <li key={tx.id}>
                                   {tx.date} | 
                                   <span className={tx.type === 'income' ? 'income-color' : 'expense-color'}>
                                       {tx.type === 'income' ? ' [입금]' : ' [출금]'}
                                   </span> 
                                   {' '}
                                   {tx.type === 'income' 
                                       ? `${getMemberName(tx.memberId)} (${tx.category})` 
